...
impl DatabaseTrait for Database {
    ...
    async fn init(env_var: &EnvironmentVariables) -> Result<Self, Error> {
        if !sqlx::Postgres::database_exists(&env_var.database_url).await? {
            tracing::info!("Database not exist. \nStarting create database.");
            sqlx::Postgres::create_database(&env_var.database_url).await?;
    ...
    async fn init_superuser(db: &Self, env_var: &EnvironmentVariables) -> Result<(), Error> {
        ...
        match super_user {
            None => {
                ...
                futures::executor::block_on(user_repo.insert(payload))
